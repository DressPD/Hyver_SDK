...
from hyver._core._base_client import _INITIAL_RETRY_DELAY, _MAX_RETRY_DELAY
...
class TestRetryOnServerError:
    ...
    def test_retries_on_500(self, base_url: str, api_key: str) -> None:
        ...
        client = HyverSDK(api_key=api_key, base_url=base_url, max_retries=1)
    ...
    def test_exhausts_retries_then_raises(self, base_url: str, api_key: str) -> None:
        ...
        client = HyverSDK(api_key=api_key, base_url=base_url, max_retries=2)
...
class TestTimeoutHandling:
    ...
    def test_timeout_raises_api_timeout_error(self, base_url: str, api_key: str) -> None:
        ...
        client = HyverSDK(api_key=api_key, base_url=base_url, max_retries=0)
...
class TestRetryDelay:
    def test_retry_after_seconds_header(self, client: HyverSDK) -> None:
        response = httpx.Response(429, headers={"retry-after": "2"})
        delay = client._retry_delay(response, 0)
        assert delay == 2.0

    def test_retry_after_capped_at_max(self, client: HyverSDK) -> None:
        response = httpx.Response(429, headers={"retry-after": "100"})
        delay = client._retry_delay(response, 0)
        assert delay == _MAX_RETRY_DELAY

    def test_exponential_backoff_without_header(self, client: HyverSDK) -> None:
